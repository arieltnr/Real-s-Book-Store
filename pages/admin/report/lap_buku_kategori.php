...
$pdf->Cell(25.5,0.7,"Laporan Data Buku - Per Kategori",0,10,'C');
$pdf->ln(1);
$pdf->SetFont('Arial','B',10);
$pdf->Cell(5,0.7,"Dicetak pada : ".date("D-d/m/Y"),0,0,'C');
$pdf->ln(1);
$pdf->Cell(4,0.7,"Kategori : ".$_GET['id'],0,0,'C');
$pdf->ln(1);
$pdf->SetFont('Arial','B',10);
...
$no=1;
$np = $_GET['id'];
$query = mysqli_query($conn, "SELECT * FROM buku JOIN jenis USING(id_jenis) JOIN penerbit USING(id_penerbit) WHERE nama_jenis = ".$np);
